...
app.factory('svsProductsOrderData', function () {
		...
		console.log("svsProductsOrderData");
		return {
			// DATA SKU
...
// SVS SEND PRODUCT CONTROLLER
 app.controller("SVSSendProduct", function($scope, $http , $document ,  $rootScope , svsPrepareSendOrder){
	...
	$scope.branches = [
		...
		{metaKey : "branch_CHCH" , city : "Christchurch " , symbol : "C" , abbreviation : "CHCH"} ,
	];
	...
	$scope.sendOrder = (ordernumber) =>{
		swal({
			title: "Would you like to send it now?",
			icon: "info",
			buttons: {
				cancel : "No",
				confirm : "Yes"
		  ...
		  .then((willSend) => {
			if (willSend) {
				var readySendDataJson =  {
					...
					OrderNumber: Number($scope.orderData.number),
