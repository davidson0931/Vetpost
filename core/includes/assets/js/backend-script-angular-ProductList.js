 ...
 app.controller("ProductList" , function($scope, $http , GetAllOrder , $timeout , $q ,  $rootScope , svsProductsOrderData , svsOrdersItemsCount , svsPrepareSendOrder, svsCancelOrder ){
	...
	$scope.request = {request: "all_orders" , page :  1 , per_page : 20 };
	
	// , wc-processing , wc-svs-processing, wc-partial , wc-completed
	...
	$scope.getapi = function(){
		...
		GetAllOrder.get($scope.request, function(obj ) {
			...
			// 		for(var k = 0; k < getitem[0].categories.length; k++){
				 
			// 			if(getitem[0].categories[k].match("Dogs|Cats") != null){
			// 			$scope.productCategories = getorder[0].line_items[j].categories
	...
	$scope.checkOrderList = function(status, line_items) {
		...
		var isAllVP = line_items.filter(item =>{
    // console.log(item);
			return !item.sku.match("VP-") && item.sku !== ""  && item.sku !== null && !item.sku.match("VP") &&
                item.branch_CHCH !== null
                && item.branch_HAMI !== null
                && item.branch_PNTH !== null;
		});
		
		
		
		/* if(isSvsExist != null && line_items.length == 1 && line_items.length === line_items.length){
			return false;
		}
		...
		if(status == "processing" && isAllVP.length == line_items.length){
			return true;
	...
	$scope.statusAll = () => {
		...
		// $scope.request.status = "ready-send , processing , svs-processing , partial , completed" ;
	...
	$scope.statuslistActive = (status) =>{
		$scope.colorValue();
		
		if($scope.request.status == status){
			return $scope.activeNavLink;
		}
		/* if("ready-send , processing , svs-processing , partial , completed" === $scope.request.status){
			return $scope.activeNavLink;
		} */
...
var getorder = $scope.orders.filter((item) => {
			return parseInt(OrderID) === parseInt(item.number);
		 }) 
   ...
   console.log(getitem , "getorder");
		if(getitem[0].branch_HAMI != null && getitem[0].branch_PNTH != null && getitem[0].branch_CHCH != null)
		{
			...
			svsProductsOrderData.setHAMIStock(Number(getitem[0].branch_HAMI.SOH));
			...
			svsProductsOrderData.setPNTHStock(Number(getitem[0].branch_PNTH.SOH));	
			...
			svsProductsOrderData.setCHCHStock(Number(getitem[0].branch_CHCH.SOH));	
		} 
